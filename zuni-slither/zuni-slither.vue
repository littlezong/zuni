<template>
	<view class="zuni-slither_item"
		...
		<view class="zuni-slither_item_actions" :style="{right: calcActionsPositionRight}">
	...
	export default{
		...
		computed: {
			...
			calcActionsPositionRight(){
				...
				const right = this.actions.map(action => parseFloat(this.calcActionStyle(action.style).width)).reduce((total, width) => total + width) * -1 + unit
				return right
			}
		},
		methods: {
			...
			handleTouchEnd(e){
				const isReached = Math.abs(this.touchTarget.offsetX) < this.touchTarget.maxToSlide / 3
				if(isReached){
					this.touchTarget.offsetX = 0
		...
		mounted(){
			...
			SLITHER_INDEX++
			this.touchTarget.maxToSlide = uni.upx2px(Math.abs(parseFloat(this.calcActionsPositionRight)))
		},
		watch: {
			activeIndex(){
				if(this.touchTarget.index !== this.activeIndex){
